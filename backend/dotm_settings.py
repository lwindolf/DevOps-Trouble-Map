...
settings = {
    'fetch_parameters':        {'description': 'Fetch method: How to fetch node infos. Possible method values are:'
                                ' "agent" (when you have installed dotm_agent) and "ssh" (when you want to'
                                ' rely on user equivalency between the DOTM server and all monitored nodes). In'
								' the last case you might want to change SSH user and key too',
                                'title': 'Fetch Method',
                                'type': 'hash',
                                'default': {
									'method': 'ssh',
